...
public class KICK2021HTransformTheString
{
    ...
    static void swap(int[] a, int idx1, int idx2){if (a[idx1] == a[idx2])return;a[idx1] ^= a[idx2];a[idx2] ^= a[idx1];a[idx1] ^= a[idx2];}
    ...
    public static void main(String[] args) throws IOException
    {
        ...
        for(int i = 1; i <= T; i++)
        {
            ...
            System.out.println("Case #"+i+": "+solver(s,p));
        }
    }
    static int solver(String s, String f)
    {
        int count = 0;
        char F = f.charAt(0);
        for(char ch : s.toCharArray())
        {
            int min1 = Math.abs(ch - F);
            int min2 = 26 - min1;
            count += (MIN(min1,min2));
        }
        return count;
